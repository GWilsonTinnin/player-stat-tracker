    ...
    this.plugin.counters.forEach((counter, index) => {
      ...
      deleteBtn.addEventListener("click", () => __async(this, null, function* () {
        if (this.plugin.settings.showDeleteConfirm) {
          if (!confirm(`Delete counter "${counter.key}"?`))
            return;
        }
...
var PlayerStatSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Counter display format").setDesc("Choose how counters are displayed").addDropdown((dropdown) => dropdown.addOption("compact", "Compact (value only)").addOption("full", "Full (name + value)").setValue(this.plugin.settings.displayFormat).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.displayFormat = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian2.Setting(containerEl).setName("Enable counter history").setDesc("Keep track of all counter changes with timestamps").addToggle((toggle) => toggle.setValue(this.plugin.settings.trackHistory).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.trackHistory = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian2.Setting(containerEl).setName("Show delete confirmation").setDesc("Ask for confirmation before deleting a counter").addToggle((toggle) => toggle.setValue(this.plugin.settings.showDeleteConfirm).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.showDeleteConfirm = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian2.Setting(containerEl).setName("Minimum counter value").setDesc("Set the minimum value counters can reach").addText((text) => text.setPlaceholder("0").setValue(String(this.plugin.settings.minValue)).onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian2.Setting(containerEl).setName("Maximum counter value").setDesc("Set the maximum value counters can reach (leave empty for unlimited)").addText((text) => text.setPlaceholder("unlimited").setValue(this.plugin.settings.maxValue ? String(this.plugin.settings.maxValue) : "").onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian2.Setting(containerEl).setName("Button style").setDesc("Choose button appearance").addDropdown((dropdown) => dropdown.addOption("default", "Default").addOption("large", "Large").addOption("minimal", "Minimal").setValue(this.plugin.settings.buttonStyle).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.buttonStyle = value;
      yield this.plugin.saveSettings();
    })));
    containerEl.createEl("hr");
    containerEl.createEl("h3", { text: "Data Management" });
    new import_obsidian2.Setting(containerEl).setName("Export counters").setDesc("Download your counter data as JSON").addButton((button) => button.setButtonText("Export").onClick(() => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Clear all counters").setDesc("WARNING: This will delete all counters permanently").addButton((button) => button.setButtonText("Clear All").setWarning().onClick(() => __async(this, null, function* () {
...
var PlayerStatCounterPlugin = class extends import_obsidian3.Plugin {
  ...
  findAndReplaceVariable(container, varRef, key) {
    ...
    while (node = walker.nextNode()) {
      ...
      if (parent == null ? void 0 : parent.classList.contains("player-stat-variable-link")) {
        continue;
      }
      const text = node.textContent || "";
    ...
    if (nodesToProcess.length > 0) {
      ...
      const wholeMatch = nodesToProcess.find(({ node: node2 }) => (node2.textContent || "").includes(varRef));
      if (wholeMatch) {
